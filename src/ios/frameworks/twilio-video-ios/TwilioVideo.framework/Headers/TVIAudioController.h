 ...
 *
 *  @discussion `TVIAudioController` can not be created with init. Use `sharedController` instead.
 */
- (null_unspecified instancetype)init __attribute__((unavailable("TVIAudioController can not be created with init")));

/**
 *  @brief Access the shared `TVIAudioController` instance.
 *
 *  @return The shared instance of `TVIAudioController.
 */
+ (_Nonnull instancetype)sharedController;
